...
# expected number of misclassified cases based on standard screening panels
...
# The draw_misclass_cohort() draws an intentionally misclassified cohort composed of (1) non-carriers labelled as 
# carrier cases, (2) miss-classified carriers that actually have CF labelled as cases and (3) non-carriers labelled 
# as controls. This function takes an integer input that defines the expected number of missclassified carriers 
# (i.e., the number of individuals labelled as carriers tnat we think might actually have CF). Note: the cases are 
...
# Finally for each condition we can compute a type of p-val, corresponding to the hypothesis that the results 
...
# The second simulation we performed was across all conditions...to assess the likelihood of obtaining similar results
# across all 59 different conditions. Specifically, we tested the hypothesis that 59 conditions could return 
# results >= the observed study results simultaneously. Note: in the actual study we ran millions of simulations. 

# For this simulation we can actually just use the results from above...we simply have to count the number of significant 
sim_res %>% 
  unnest() %>% 
  gather(key=name,value=sim_or,-trial) %>% 
  inner_join(original_results) %>% 
...
# For the third simulation we tested the hypothesis that the results we generated could be due to "False Discovery" by 
# perfoming  multiple statistical tests. Because we ran 59 different statistical tests, it may be the case that some of 
# these results could have occured by chance. To test this possibility we simply repeatedly draw study cohorts, using 
# the functions above, where we do not include missclassified Carriers (i.e., both cases and controls are non-carriers).
# We then re-run our analysis and summarize the number of significant results we return.

# This simulation takes a long-time to run, and to speed up the process, we use parallel computing. The results of this
# simulation are also included in the results file loaded above.
# To regenerate these results uncomment the following.
...
# Using the dataset "sim_res2", that was loaded above, we can now compute an empirical false discovery rate...
# note based on this overly simple analysis we only obtain ~2-3 significant results. However, in the actual study we 
# performed an analysis that was slightly more complex...so the results are slightly different.
sim_res2 %>% 
  unnest() %>% 
  group_by(trial) %>% 
